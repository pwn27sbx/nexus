...
const UserProfile = ({ isOpen, onClose, user, onLogout, accentColor, setAccentColor, fontType, setFontType }) => {
  ...
  const handleSaveNickname = async () => {
    ...
    try {
      ...
      setNameError('Network error. Try again.'); setNickname(user.nickname || '');
      ...
      <div className="fixed top-0 right-0 z-[101] w-full max-w-md h-full bg-[#fafafa] dark:bg-[#050505] shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="relative group flex flex-col items-center justify-center w-full max-w-[240px]">
            <div className="flex items-center relative w-full">
              ...
              <input
                ...
                onChange={(e) => { setNickname(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '')); setNameError(''); }}
                onBlur={handleSaveNickname} onKeyDown={(e) => e.key === 'Enter' && e.target.blur()} placeholder="Set a nickname..."
                className="w-full text-center text-2xl font-bold tracking-tight text-black dark:text-white bg-transparent border-b-2 border-transparent focus:border-accent hover:border-black/5 dark:hover:border-white/5 outline-none transition-colors placeholder:text-zinc-300 dark:placeholder:text-zinc-800 px-8 py-1"
              />
        ...
        <div className="flex gap-8 px-8 pt-6 bg-[#fafafa] dark:bg-[#050505]">
          <button onClick={() => setActiveTab('arsenal')} className={`pb-4 text-[14px] font-semibold transition-all relative ${activeTab === 'arsenal' ? 'text-black dark:text-white' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}>
            My Arsenal
            {activeTab === 'arsenal' && <div className="absolute bottom-0 left-0 w-full h-[2px] rounded-t-full transition-colors duration-300 bg-accent" />}
          </button>
          <button onClick={() => setActiveTab('submissions')} className={`pb-4 text-[14px] font-semibold transition-all relative ${activeTab === 'submissions' ? 'text-black dark:text-white' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}>
            My Submissions
            {activeTab === 'submissions' && <div className="absolute bottom-0 left-0 w-full h-[2px] rounded-t-full transition-colors duration-300 bg-accent" />}
          </button>
        </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3.5">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[14px] font-bold text-zinc-900 dark:text-white group-hover:text-accent transition-colors leading-tight">{tool.name}</span>
                      ...
                      <div className="w-8 h-8 rounded-full bg-zinc-50 dark:bg-[#161616] flex items-center justify-center text-zinc-400 transition-colors shrink-0 hover-bg-accent hover:text-white dark:hover-bg-accent dark:hover:text-black">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="7" y1="17" x2="17" y2="7"></line><polyline points="7 7 17 7 17 17"></polyline></svg>
                      </div>
                    </div>
                  </a>
                ))
            ...
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-white dark:bg-[#111] border border-black/5 dark:border-white/5 rounded-2xl p-4 flex flex-col justify-between shadow-sm">
                  ...
                  <span className="text-2xl font-bold text-black dark:text-white mt-1">{totalSubmissions}</span>
                </div>
                ...
                <div className="bg-white dark:bg-[#111] border border-black/5 dark:border-white/5 rounded-2xl p-4 flex flex-col justify-between shadow-sm">
                  <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest font-mono">Win Rate</span>
                  <span className="text-2xl font-bold text-black dark:text-white mt-1">{successRate}%</span>
                </div>
              </div>

              {submissions.length === 0 ? (
                <div className="py-10 text-center border-t border-black/5 dark:border-white/5">
                ...
                <div className="space-y-3">
                  {submissions.map(tool => (
                    <div key={tool.id} className="p-3 rounded-2xl bg-white dark:bg-[#111] border border-black/5 dark:border-white/5 flex items-center justify-between">
                      ...
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1.5 rounded-md ${tool.status === 'approved' ? 'bg-accent/10 text-accent' : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400'}`}>
                        {tool.status}
                      </span>
                    </div>
                  ))}
