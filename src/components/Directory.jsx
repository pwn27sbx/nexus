...
const HeartIcon = ({ isSaved }) => (
  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill={isSaved ? "#ff8787" : "none"} stroke={isSaved ? "#ff8787" : "currentColor"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="transition-all duration-300">
      ...
      <div className="bg-white dark:bg-[#0f0f11] w-full max-w-2xl rounded-2xl shadow-2xl border border-black/10 dark:border-white/10 overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <div className="flex items-center px-5 py-5 border-b border-black/5 dark:border-white/5">
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none text-black dark:text-white px-4 text-xl font-medium placeholder:text-zinc-300 dark:placeholder:text-zinc-700"
          />
            ...
            <div className="mb-4">
              ...
              {filteredTools.map(tool => (
                <button key={tool.id} onClick={() => { window.open(tool.url, '_blank'); onClose(); }} className="w-full text-left px-4 py-3 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 flex items-center justify-between group transition-all">
                  <span className="text-[15px] text-zinc-900 dark:text-white font-semibold">{tool.name}</span>
                  <span className="text-[12px] font-mono text-zinc-400 opacity-0 group-hover:opacity-100 transition-opacity">Visit ↵</span>
                </button>
              ))}
            </div>
          )}
            ...
            <div>
              ...
              {(query ? filteredCommands : commands).map(cmd => (
                <button key={cmd.id} onClick={() => { cmd.action(); onClose(); }} className="w-full text-left px-4 py-3 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 flex items-center justify-between group transition-all">
                  <span className="text-[15px] text-zinc-900 dark:text-white font-medium">{cmd.title}</span>
                  <span className="text-[12px] font-mono text-zinc-400 opacity-0 group-hover:opacity-100 transition-opacity">Execute ↵</span>
                </button>
              ))}
            </div>
          )}

          {query && filteredTools.length === 0 && filteredCommands.length === 0 && (
            <div className="py-12 text-center">
              <p className="text-[14px] text-zinc-500 font-medium">No results found for "{query}"</p>
            </div>
          )}
        </div>
      </div>
...
const MasonryCard = ({ tool, user, onRequireAuth, isFocused }) => {
  ...
  const handleToggleSave = async (e) => {
    ...
    if (isSaved) { newBookmarks = newBookmarks.filter(id => id !== numericToolId); }
    else { newBookmarks.push(numericToolId); }

    try {
      const res = await fetch(`https://nexus-production-8dca.up.railway.app/api/users/${user.id}`, {
        method: 'PATCH',
  ...
  const handleCapture = async (e) => {
    ...
    try {
      const res = await fetch('https://nexus-production-8dca.up.railway.app/api/tools', {
        ...
        body: JSON.stringify({ name, url, category, submittedBy: user ? user.id : null }),
      });
      if (res.ok) {
        setSubmitStatus('success');
        setTimeout(() => { onClose(); setName(''); setUrl(''); setCategory('Design'); setSubmitStatus('idle'); }, 3000);
      ...
      <div className="bg-white dark:bg-[#111] border border-black/10 dark:border-white/10 w-[90%] max-w-md rounded-2xl p-6 shadow-2xl relative">
        ...
        {submitStatus === 'success' ? (
          <div className="py-8 flex flex-col items-center justify-center text-center animate-in fade-in zoom-in duration-300">
            <div className="w-12 h-12 bg-green-500/10 text-green-500 rounded-full flex items-center justify-center mb-3"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></div>
            <h3 className="text-black dark:text-white font-medium mb-1">Tool Submitted!</h3><p className="text-[13px] text-zinc-500">Waiting for approval.</p>
          </div>
        ) : (
...
export default function App() {
  ...
  useEffect(() => {
    const checkUserSession = async () => {
      ...
      if (!token) return;
      try {
        const res = await fetch('https://nexus-production-8dca.up.railway.app/api/users/me', { headers: { 'Authorization': `JWT ${token}` } });
        if (res.ok) { const data = await res.json(); setUser(data.user); } else { localStorage.removeItem('payload-token'); }
      } catch (err) {}
    };
    checkUserSession();
  }, [isAuthModalOpen]);
      ...
      try {
        ...
        const fetchedTools = hits.map((tool, index) => {
          ...
          return {
            ...
            heightClass: assignedHeight, actionText: "View Production",
          };
        });
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] dark:bg-[#050505] transition-colors duration-300 font-sans selection:bg-zinc-300 dark:selection:bg-zinc-700 pb-32">
      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-40 w-[90%] max-w-lg bg-white/80 dark:bg-[#111]/80 backdrop-blur-xl border border-black/5 dark:border-white/10 rounded-full shadow-lg flex items-center justify-between p-1.5 transition-all">
        ...
        >
          <SearchIcon />
          <span className="ml-2 text-zinc-500 text-[12px] font-medium text-left flex-1 group-hover:text-zinc-700 dark:group-hover:text-zinc-300 transition-colors truncate">
            {searchQuery ? searchQuery : "Search or type a command..."}
          </span>
        ...
        <div className="flex gap-1 pr-1">
          ...
          {user ? (
            <button onClick={() => setIsProfileOpen(true)} className="px-3 h-8 rounded-full border border-black/10 dark:border-white/10 text-black dark:text-white hover:bg-black/5 dark:hover:bg-white/10 text-[11px] font-medium transition-all active:scale-95 cursor-pointer flex items-center gap-1.5">
              <div className="w-1.5 h-1.5 rounded-full bg-green-500"></div>{user.nickname ? `@${user.nickname}` : user.email.split('@')[0]}
            </button>
          ) : (
      ...
      <div className="pt-28 pb-8 flex justify-center w-full z-30 sticky top-0 bg-gradient-to-b from-[#fafafa] dark:from-[#050505] to-transparent pointer-events-none">
        <div className="flex gap-2 p-1.5 rounded-full bg-white/50 dark:bg-[#111]/50 backdrop-blur-md border border-black/5 dark:border-white/5 pointer-events-auto overflow-x-auto max-w-[95%] no-scrollbar">
          {["All", "Design", "Development", "AI Tools", "Productivity"].map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 py-1.5 rounded-full text-[12px] font-medium transition-all whitespace-nowrap ${activeCategory === cat ? 'bg-black text-white dark:bg-white dark:text-black shadow-sm' : 'text-zinc-500 dark:text-zinc-400 hover:text-black dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}>{cat}</button>
          ))}
        </div>
      </div>
