...
const AutoCaptureModal = ({ isOpen, onClose, user }) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/tools`, {
        ...
        body: JSON.stringify({
          ...
          tags: selectedTags, status: 'pending',
  ...
  const inputStyle = {
    ...
    borderRadius: '16px', padding: '14px 18px', outline: 'none',
  ...
  return (
    <div
      ...
      onClick={onClose}
      role="dialog" aria-modal="true" aria-label="Suggest a tool"
    >
      {/* Close pill */}
      <button
        onClick={onClose}
        ...
        >
          ...
          <div style={{
            ...
            <div style={{
              ...
              display: 'flex', alignItems: 'center', paddingLeft: '10px', overflow: 'hidden',
            }}>
              <span style={{ fontSize: '10px', color: isDark ? 'rgba(180,160,255,0.35)' : 'rgba(100,80,180,0.45)', fontFamily: 'monospace', whiteSpace: 'nowrap' }}>
                {previewUrl ? previewUrl.replace(/^https?:\/\/(www\.)?/, '').split('/')[0] : 'preview.site'}
              </span>
            </div>
          </div>
          {/* Preview content */}
          <div style={{
            flex: 1, borderRadius: '0 0 1.25rem 1.25rem', overflow: 'hidden',
            ...
            {previewUrl ? (
              ...
              <div style={{ textAlign: 'center', padding: '40px' }}>
                <div style={{ width: '64px', height: '64px', borderRadius: '18px', background: isDark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: '28px' }}>🌐</div>
                <p style={{ fontSize: '14px', fontWeight: 600, color: isDark ? 'rgba(180,165,240,0.5)' : 'rgba(100,80,160,0.45)' }}>Enter a URL to see a live preview</p>
                <p style={{ fontSize: '12px', fontWeight: 500, marginTop: '6px', color: isDark ? 'rgba(160,145,220,0.35)' : 'rgba(120,100,180,0.35)' }}>The website will appear here</p>
              </div>
            )}
          </div>
        </div>
          ...
          {submitStatus === 'success' ? (
            ...
            <form onSubmit={handleCapture}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
                <input type="text" required placeholder="Tool Name" value={name} onChange={(e) => setName(e.target.value)} disabled={submitStatus === 'loading'} style={inputStyle} aria-label="Tool name" />
                <div>
                  <div style={{ ...inputStyle, display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 18px' }}>
                    <GlobeIcon />
                    <input type="url" required placeholder="https://example.com" value={url} onChange={handleUrlChange} disabled={submitStatus === 'loading'} style={{ background: 'transparent', border: 'none', outline: 'none', width: '100%', fontSize: '14px', fontWeight: 600, color: isDark ? 'rgba(235,230,255,0.9)' : 'rgba(20,15,50,0.85)' }} aria-label="Tool URL" />
                  </div>
                  {urlError && <p style={{ color: '#ef4444', fontSize: '11px', fontWeight: 600, marginTop: '6px', marginLeft: '4px' }}>{urlError}</p>}
                </div>
                <div style={{ ...inputStyle, display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '12px 18px' }}>
                  <div style={{ marginTop: '2px' }}><DescIcon /></div>
                  <input type="text" required maxLength={APP_CONFIG.MAX_DESCRIPTION_LENGTH} placeholder="Brief description" value={description} onChange={(e) => setDescription(e.target.value)} disabled={submitStatus === 'loading'} style={{ background: 'transparent', border: 'none', outline: 'none', width: '100%', fontSize: '14px', fontWeight: 600, color: isDark ? 'rgba(235,230,255,0.9)' : 'rgba(20,15,50,0.85)' }} aria-label="Description" />
                </div>
                <div style={{ ...inputStyle, padding: '12px 16px' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: selectedTags.length > 0 ? '10px' : '0' }}>
                    ...
                    {selectedTags.length < 5 && (
                      <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addTag(tagInput); } else if (e.key === ',' || e.key === 'Tab') { e.preventDefault(); addTag(tagInput.replace(',', '')); } }} placeholder="Add tag..." disabled={submitStatus === 'loading'} style={{ background: 'transparent', border: 'none', outline: 'none', fontSize: '12px', fontWeight: 600, minWidth: '80px', flex: 1, color: isDark ? 'rgba(235,230,255,0.9)' : 'rgba(20,15,50,0.85)' }} />
                    )}
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {COMMON_TAGS.filter((t) => !selectedTags.includes(t.id)).slice(0, 6).map((t) => (
                      <button key={t.id} type="button" onClick={() => addTag(t.id)} disabled={submitStatus === 'loading'} style={{ padding: '2px 8px', borderRadius: '100px', background: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)', color: isDark ? 'rgba(180,165,240,0.6)' : 'rgba(80,60,140,0.55)', fontSize: '10px', fontWeight: 600, border: 'none', cursor: 'pointer', transition: 'all 0.15s ease' }}>+ {t.label}</button>
                    ))}
                  </div>
                </div>
                <select value={category} onChange={(e) => setCategory(e.target.value)} disabled={submitStatus === 'loading'} style={{ ...inputStyle, appearance: 'none', cursor: 'pointer' }} aria-label="Category">
